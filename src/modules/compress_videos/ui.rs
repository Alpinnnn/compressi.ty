...
use eframe::egui::{
    self, Align, Button, Color32, ColorImage, CornerRadius, Id, Layout, RichText, ScrollArea,
    Sense, Slider, Stroke, StrokeKind, TextureHandle, TextureOptions, Ui, vec2, pos2, Rect,
    DragValue,
};
...
/// Video compression workspace — queue-based batch system.
...
fn flush(ui: &mut Ui) { ui.spacing_mut().item_spacing = vec2(0.0, 0.0); }
fn compact(ui: &mut Ui) { ui.spacing_mut().item_spacing = vec2(8.0, 8.0); }

fn truncate_filename(name: &str, max_chars: usize) -> String {
    if name.len() <= max_chars { return name.to_owned(); }
    if let Some(dot_pos) = name.rfind('.') {
        let ext = &name[dot_pos..];
        let stem_budget = max_chars.saturating_sub(ext.len()).saturating_sub(1);
        if stem_budget >= 4 { return format!("{}…{}", &name[..stem_budget], ext); }
    }
...
impl CompressVideosPage {
    pub fn is_compressing(&self) -> bool { self.active_batch.is_some() }

    pub fn cancel_compression(&self) {
        if let Some(batch) = &self.active_batch { batch.cancel(); }
    }
                    ...
                    BatchEvent::VideoStarted { id } => {
                        if let Some(item) = self.queue.iter_mut().find(|i| i.id == id) {
                            item.state = VideoCompressionState::Compressing(ProcessingProgress {
                                progress: 0.02, stage: "Starting".to_owned(),
                                speed_x: 0.0, eta_secs: None,
                            });
                        }
                    }
    ...
    fn add_paths(&mut self, paths: Vec<PathBuf>) {
        ...
        let new_paths: Vec<PathBuf> = paths.into_iter()
            .filter(|p| processor::is_supported_video_path(p))
            .filter(|p| !self.queue.iter().any(|i|
                i.source_path == *p && matches!(i.state, VideoCompressionState::Ready | VideoCompressionState::Probing)
            ))
            .collect();
            ...
            thread::spawn(move || {
                ...
                let thumbnail = if let Ok(ref meta) = metadata_result {
                    processor::generate_thumbnail(&engine_clone, &path, meta.duration_secs)
                        .ok()
                        .map(|(rgba, width, height)| VideoThumbnail { rgba, width, height })
                } else {
                    None
                };
                let _ = tx.send(ProbeResult { metadata: metadata_result, thumbnail });
            });
        }
    ...
    fn start_batch_compression(&mut self) {
        ...
        let items: Vec<BatchItem> = self.queue.iter()
            ...
            .filter_map(|i| {
                ...
                Some(BatchItem { id: i.id, video, settings })
            })
            .collect();
        ...
        match processor::start_video_batch(engine, items, self.output_dir.clone()) {
            ...
            Err(e) => {
                self.banner = Some(BannerMessage { tone: BannerTone::Error, text: e });
            }
        }
    }
    ...
    pub fn show(
        ...
        let page_margin = if avail >= 1280.0 { 28.0 }
            else if avail >= 960.0 { 22.0 }
            else if avail >= 720.0 { 16.0 }
            else { 12.0 };
            ...
            content_ui.allocate_ui_with_layout(
                ...
                |ui| {
                    ...
                    ui.allocate_ui_with_layout(
                        ...
                        |ui| { flush(ui); self.render_queue(ui, theme, workspace_h); },
                    );
                    ui.add_space(gutter);
                    ui.allocate_ui_with_layout(
    ...
    fn render_toolbar(&mut self, ui: &mut Ui, theme: &AppTheme, active_module: &mut Option<ModuleKind>) {
        ...
        panel::card(theme)
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal_wrapped(|ui| {
                    if ui.add(
                        Button::new(RichText::new(format!("{} Back", icons::BACK)).size(13.0).color(theme.colors.fg))
                    ...
                    ui.label(RichText::new("Compress Videos").size(20.0).strong().color(theme.colors.fg));

                    ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                        if ui.add(
                            Button::new(RichText::new(format!("{} Change Output", icons::FOLDER)).size(13.0).strong().color(Color32::BLACK))
                                .fill(theme.colors.accent).stroke(Stroke::NONE).corner_radius(CornerRadius::ZERO),
                        ).clicked() {
                        ...
                        if let Some(dir) = &output_dir {
                            if ui.add(
                                Button::new(RichText::new(format!("{} Open Output", icons::FOLDER)).size(13.0).color(theme.colors.fg))
        ...
        match self.engine_status.clone() {
            ...
            EngineStatus::Failed(error) => {
                panel::tinted(theme, theme.colors.negative).show(ui, |ui| {
                    ui.label(RichText::new("Video tools could not be prepared").size(13.0).strong().color(theme.colors.fg));
        ...
        ui.allocate_ui_with_layout(vec2(ui.available_width(), height.max(0.0)), Layout::top_down(Align::Min), |ui| {
            panel::card(theme).fill(fill).stroke(stroke).inner_margin(egui::Margin::same(18)).show(ui, |ui| {
                ...
                ui.vertical_centered(|ui| {
                    ui.label(RichText::new(if has_files {
                        let ready_count = self.queue.iter().filter(|i| matches!(i.state, VideoCompressionState::Ready)).count();
                        format!("{} video(s) ready. Drop more videos here.", ready_count)
                    } else {
                        "Drop videos here to start your workspace".to_owned()
                    }).size(if has_files { 13.0 } else { 16.0 }).strong().color(theme.colors.fg));
                    ui.add_space(8.0);
                    if ui.add_enabled(ready, Button::new(
                    ...
                    if !ready {
                        ui.add_space(4.0);
                        ui.label(RichText::new("Video tools are being prepared…").size(11.0).color(theme.colors.fg_dim));
                    }
                    // Probing status
                    if !self.pending_probes.is_empty() {
                        ui.add_space(6.0);
                        ui.label(RichText::new(format!("Probing {} video(s)…", self.pending_probes.len())).size(11.0).color(theme.colors.fg_dim));
                    }
                });
            });
        });
    ...
    fn render_queue(&mut self, ui: &mut Ui, theme: &AppTheme, height: f32) {
        ...
        panel::card(theme).inner_margin(egui::Margin::same(14)).show(ui, |ui| {
            ...
            ScrollArea::vertical().id_salt("video_queue_scroll").auto_shrink([false, false]).max_height(scroll_h).show(ui, |ui| {
                flush(ui);
                ui.set_width(ui.available_width());

                let categories: [(& str, Color32, Box<dyn Fn(&VideoCompressionState) -> bool>); 4] = [
                    ("Probing", theme.colors.fg_muted, Box::new(|s| matches!(s, VideoCompressionState::Probing))),
                    ("Queue", theme.colors.fg_muted, Box::new(|s| matches!(s, VideoCompressionState::Ready))),
                    ("Progress", theme.colors.accent, Box::new(|s| matches!(s, VideoCompressionState::Compressing(_)))),
                    ("Done", theme.colors.positive, Box::new(|s| matches!(s, VideoCompressionState::Completed(_) | VideoCompressionState::Failed(_) | VideoCompressionState::Cancelled))),
                ];

                for (title, tint, filter) in &categories {
                    let indices: Vec<usize> = self.queue.iter().enumerate()
                        .filter(|(_, i)| filter(&i.state)).map(|(idx, _)| idx).collect();
                    ...
                    for &idx in &indices {
                        ...
                        let can_delete = !is_compressing && matches!(item.state, VideoCompressionState::Ready | VideoCompressionState::Failed(_) | VideoCompressionState::Cancelled);
                        let thumb_tex = self.thumbnail_textures.get(&item.id);
                        let action = video_queue_row(ui, theme, item, selected, can_delete, thumb_tex);
                        if action.clicked { clicked_id = Some(item.id); }
                        if action.deleted { delete_id = Some(item.id); }
                    }
                    ui.add_space(8.0);
                }
            });
        ...
        if let Some(id) = clicked_id {
            self.selected_id = if self.selected_id == Some(id) { None } else { Some(id) };
        }
    }
    ...
    fn render_settings_panel(&mut self, ui: &mut Ui, theme: &AppTheme, height: f32) {
        panel::card(theme).inner_margin(egui::Margin::same(14)).show(ui, |ui| {
            ...
            ui.add_space(4.0);
            ui.label(RichText::new(format!("{} | {} | {}×{}", format_bytes(metadata.size_bytes), format_duration(metadata.duration_secs), metadata.width, metadata.height))
            ...
            ScrollArea::vertical().id_salt("video_settings_scroll").auto_shrink([false, false]).max_height((height - 100.0).max(0.0)).show(ui, |ui| {
                ...
                // Mode selector — card style (matching compress_photos design)
                ...
                match settings.mode {
                    ...
                    CompressionMode::CustomAdvanced => {
                        ui.label(RichText::new("Bitrate").size(12.0).color(theme.colors.fg_dim));
                        ui.add(DragValue::new(&mut settings.custom_bitrate_kbps).range(350..=80_000).speed(50.0).suffix(" kbps"));
                        ui.add_space(4.0);
                        ...
                        ui.horizontal_wrapped(|ui| {
                            for codec in CodecChoice::ALL {
                                ...
                                if ui.add_enabled(enabled, Button::new(RichText::new(codec.label()).size(11.5).color(theme.colors.fg))
                                    .fill(if settings.custom_codec == codec { theme.mix(theme.colors.bg_raised, theme.colors.accent, 0.18) } else { theme.colors.bg_raised })
                                    .stroke(Stroke::new(1.0, theme.colors.border)).corner_radius(CornerRadius::ZERO)
                                ).clicked() { settings.custom_codec = codec; }
                            }
                        });
                        ui.add_space(4.0);
                        ui.label(RichText::new("Resolution").size(12.0).color(theme.colors.fg_dim));
                        ...
                        ui.add_space(4.0);
                        ui.label(RichText::new("FPS").size(12.0).color(theme.colors.fg_dim));
                        ui.add(DragValue::new(&mut settings.custom_fps).range(12..=metadata.fps.round().max(12.0) as u32).speed(1.0));
                    }
                }

                // Estimate — displays positive savings or size increase
    ...
    fn render_actions(&mut self, ui: &mut Ui, theme: &AppTheme, height: f32) {
        ...
        panel::card(theme).inner_margin(egui::Margin::same(14)).show(ui, |ui| {
            ...
            if ui.add_enabled(can_go, Button::new(
                RichText::new(format!("{} Compress All", icons::PLAY)).size(13.0).strong().color(Color32::BLACK),
            ...
            ui.horizontal(|ui| {
                if ui.add_enabled(self.active_batch.is_some(),
                    Button::new(RichText::new("Cancel").size(12.0).color(theme.colors.fg))
                        ...
                        .stroke(Stroke::new(1.0, theme.mix(theme.colors.border, theme.colors.caution, 0.24)))
                        .corner_radius(CornerRadius::ZERO),
                ).clicked() {
                    if let Some(batch) = &self.active_batch { batch.cancel(); }
                }
                if ui.add_enabled(self.active_batch.is_none() && !self.queue.is_empty(),
                    Button::new(RichText::new("Clear All").size(12.0).color(theme.colors.fg))
                        .fill(theme.mix(theme.colors.surface, theme.colors.negative, 0.08))
                        .stroke(Stroke::new(1.0, theme.mix(theme.colors.border, theme.colors.negative, 0.2)))
                        .corner_radius(CornerRadius::ZERO),
                ).clicked() {
                    self.queue.clear();
            ...
            if self.active_batch.is_some() {
                ui.horizontal(|ui| {
                    ...
                    ui.label(RichText::new("Compressing…").size(11.0).color(theme.colors.fg_dim));
                });
            }
        });
    }
...
fn render_banner(ui: &mut Ui, theme: &AppTheme, banner: &BannerMessage) {
    ...
    panel::tinted(theme, tint).inner_margin(egui::Margin::symmetric(20, 12)).show(ui, |ui| {
        ui.label(RichText::new(&banner.text).size(12.5).color(theme.colors.fg));
    });
}
...
fn queue_section_header(ui: &mut Ui, theme: &AppTheme, title: &str, count: usize, tint: Color32) {
    ui.add_space(4.0);
    ui.horizontal(|ui| {
        ui.label(RichText::new(format!("{title} — {count}")).size(12.0).strong().color(tint));
    });
    let w = ui.available_width();
    let (line_rect, _) = ui.allocate_exact_size(vec2(w, 1.0), Sense::hover());
    ui.painter().rect_filled(line_rect, CornerRadius::ZERO, theme.mix(theme.colors.border, tint, 0.30));
    ui.add_space(4.0);
}

struct QueueRowAction { clicked: bool, deleted: bool }

fn video_queue_row(ui: &mut Ui, theme: &AppTheme, item: &VideoQueueItem, selected: bool, can_delete: bool, thumb_tex: Option<&TextureHandle>) -> QueueRowAction {
    ...
    let btn_pos = pos2(row_rect.right() - btn_size.x - 8.0, row_rect.center().y - btn_size.y * 0.5);
    let btn_rect = Rect::from_min_size(btn_pos, btn_size);
    let btn_resp = if can_delete { Some(ui.interact(btn_rect, row_id.with("trash"), Sense::click())) } else { None };
    let row_hovered = row_resp.hovered() || btn_resp.as_ref().map(|b| b.hovered()).unwrap_or(false);

    let fill = if selected { theme.mix(theme.colors.bg_raised, theme.colors.accent, 0.10) } else { theme.colors.bg_raised };
    ui.painter().rect_filled(row_rect, CornerRadius::ZERO, fill);
    ui.painter().rect_stroke(row_rect, CornerRadius::ZERO, Stroke::new(1.0,
        if row_hovered || selected { theme.colors.border_focus } else { theme.colors.border }), StrokeKind::Middle);
    ...
    ui.painter().rect_stroke(thumb_rect, CornerRadius::ZERO, Stroke::new(1.0, theme.colors.border), StrokeKind::Middle);
    ...
    let text_right = if can_delete && row_hovered { btn_rect.left() - 4.0 } else { row_rect.right() - 10.0 };
    let text_w = (text_right - text_x).max(0.0);
    let mut y = row_rect.top() + 10.0;

    // File name
    let galley = ui.painter().layout_no_wrap(truncate_filename(&item.file_name, 28), egui::FontId::proportional(12.0), theme.colors.fg);
    ...
    match &item.state {
        VideoCompressionState::Probing => {
            let g = ui.painter().layout_no_wrap("Probing…".to_owned(), egui::FontId::proportional(10.0), theme.colors.fg_muted);
            ui.painter().galley(pos2(text_x, y), g, theme.colors.fg_muted);
        }
        VideoCompressionState::Ready => {
            if let Some(meta) = &item.metadata {
                let info = format!("{} | {}", format_bytes(meta.size_bytes), format_duration(meta.duration_secs));
                let g = ui.painter().layout(info, egui::FontId::proportional(10.0), theme.colors.fg_dim, text_w);
                ui.painter().galley(pos2(text_x, y), g, theme.colors.fg_dim);
            } else {
                let g = ui.painter().layout_no_wrap("Ready".to_owned(), egui::FontId::proportional(10.0), theme.colors.fg_muted);
                ui.painter().galley(pos2(text_x, y), g, theme.colors.fg_muted);
            }
        }
        VideoCompressionState::Compressing(p) => {
            let g = ui.painter().layout_no_wrap(format!("Compressing {:.0}%", p.progress * 100.0), egui::FontId::proportional(10.0), theme.colors.accent);
            ...
            if p.progress > 0.0 {
                let fill_rect = Rect::from_min_size(bar_rect.min, vec2(bar_rect.width() * p.progress.clamp(0.0, 1.0), 4.0));
                ui.painter().rect_filled(fill_rect, CornerRadius::same(2), theme.colors.accent);
            }
        }
        VideoCompressionState::Completed(r) => {
            let text = format!("Done, {} → {} ({:.1}%)", format_bytes(r.original_size_bytes), format_bytes(r.output_size_bytes), r.reduction_percent.abs());
            let g = ui.painter().layout(text, egui::FontId::proportional(10.0), theme.colors.positive, text_w);
            ui.painter().galley(pos2(text_x, y), g, theme.colors.positive);
        }
        VideoCompressionState::Failed(err) => {
            let g = ui.painter().layout(format!("Failed: {err}"), egui::FontId::proportional(10.0), theme.colors.negative, text_w);
            ui.painter().galley(pos2(text_x, y), g, theme.colors.negative);
        }
        VideoCompressionState::Cancelled => {
            let g = ui.painter().layout_no_wrap("Cancelled".to_owned(), egui::FontId::proportional(10.0), theme.colors.caution);
    ...
    if let Some(btn) = &btn_resp {
        if row_hovered {
            ...
            ui.painter().rect_stroke(btn_rect, CornerRadius::ZERO, Stroke::new(1.0, theme.mix(theme.colors.border, theme.colors.negative, 0.3)), StrokeKind::Middle);
            ui.painter().text(btn_rect.center(), egui::Align2::CENTER_CENTER, icons::TRASH, icons::font_id(13.0), theme.mix(theme.colors.negative, Color32::WHITE, 0.2 + t * 0.3));
            if btn.clicked() { action.deleted = true; }
        }
    }

    if row_resp.clicked() && !action.deleted { action.clicked = true; }
    action
}

fn choice_button(ui: &mut Ui, theme: &AppTheme, label: &str, selected: bool) -> egui::Response {
    ui.add(Button::new(RichText::new(label).size(11.5).color(theme.colors.fg))
        .fill(if selected { theme.mix(theme.colors.bg_raised, theme.colors.accent, 0.18) } else { theme.colors.bg_raised })
        .stroke(Stroke::new(1.0, theme.colors.border)).corner_radius(CornerRadius::ZERO))
}
...
fn render_simple_bar(ui: &mut Ui, theme: &AppTheme, progress: f32, label: &str) {
    if !label.is_empty() { ui.label(RichText::new(label).size(11.5).color(theme.colors.fg_dim)); ui.add_space(4.0); }
    let width = ui.available_width().max(180.0);
    let (rect, _) = ui.allocate_exact_size(vec2(width, 10.0), Sense::hover());
    ui.painter().rect_filled(rect, CornerRadius::same(2), theme.colors.bg_base);
    ui.painter().rect_stroke(rect, CornerRadius::same(2), Stroke::new(1.0, theme.colors.border), StrokeKind::Middle);
    let fill_rect = Rect::from_min_size(rect.min, vec2(rect.width() * progress.clamp(0.0, 1.0), rect.height()));
    ui.painter().rect_filled(fill_rect, CornerRadius::same(2), theme.colors.accent);
}
...
fn format_bytes(bytes: u64) -> String {
    ...
    while value >= 1024.0 && unit_index < UNITS.len() - 1 { value /= 1024.0; unit_index += 1; }
    if unit_index == 0 { format!("{} {}", bytes, UNITS[unit_index]) } else { format!("{value:.1} {}", UNITS[unit_index]) }
}
...
fn format_duration(seconds: f32) -> String {
    ...
    let secs = seconds % 60;
    if hours > 0 { format!("{hours}h {minutes}m") } else if minutes > 0 { format!("{minutes}m {secs}s") } else { format!("{secs}s") }
}
